 ...
 */
export const MissingOptions = dedupingMixin(
  (superClass) =>
    class extends EtoolsLogsMixin(superClass) {
      ...
      requestMissingOptions(notFoundValues) {
        ...
        const etoolsAjax = this.$.missingOptionsAjax;
        if (!etoolsAjax) {
          return;
        }
        etoolsAjax.url = null;
      ...
      handleMissingOptionsReqResponse(response) {
        ...
        try {
          ...
          this.logError(
            'Error occurred on missing options request response handling',
            'etools-esmm ' + this.label,
            error,
            true
          );
        ...
        if (this.options instanceof Array && this.options.length > 0) {
          ...
          receivedOptionsValues.forEach((optVal) => {
            ...
            if (!option) {
              ...
              missingOptions.push(
                receivedOptions.find((receivedOption) => {
